"""Tests pour le système d'alerte simple (EPIC-04)."""
from datetime import datetime
from fastapi.testclient import TestClient
from app.core.storage import alerts_store, events_store, get_alerts
...
def test_get_alerts_empty() -> None:
...
def test_alert_generation_after_event() -> None:
...
def test_alert_no_match() -> None:
    ...
    # Vérifier qu'aucune alerte n'a été générée
    alerts_response = client.get("/api/v1/alerts/")
    assert alerts_response.status_code == 200
    assert alerts_response.json() == []


def test_alert_rule_matching() -> None:
...
def test_generate_endpoint_no_match() -> None:
...
def test_generate_endpoint_invalid_payload() -> None:
    """Teste POST /api/v1/alerts/generate avec un payload invalide (champ manquant)."""
    payload = {
        "endpoint_id": "ep-001",
        # timestamp manquant
        "event_type": "intrusion",
...
def test_multiple_alerts_generation() -> None:
    """Teste la génération de plusieurs alertes."""
    # Premier événement - intrusion
    payload1 = {
        "endpoint_id": "ep-001",
        "timestamp": datetime.now().isoformat(),
        "event_type": "intrusion",
